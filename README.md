...
- **MIDI instrument tracks** -- compose melodies, basslines, and chord progressions with 128 General MIDI instruments. Inline MiniPianoRoll in the track drawer renders the whole track at once with the selected clip bright and other clips dimmed, so you can see and edit notes across every clip without re-selecting from the timeline. Taps land in whichever clip contains them; gap regions between clips are visibly darkened as non-writeable zones. Long-press a note body to grab and drag, or its right edge to resize, with a tactile latch the moment the gesture engages; quick swipes still pan the timeline. Notes preview audibly at the track's selected instrument as you place or drag them, whether the transport is playing or paused. Full-screen hardware-style instrument browser with a category sidebar (KEYS, STR, BRASS, BASS, PAD, LEAD, DRUM, FX, OTHER), latched-bevel patch cards, and audible audition preview across all 128 GM programs. Compact note visualization on the Studio timeline. Powered by FluidSynth synthesis via a dedicated C++ MidiSequencer.
- **Full-screen piano roll** -- a redesigned hardware-style editor that reads as a Roland or Korg groovebox faceplate. A captioned top bar (BACK, RESTART, PLAY) sits above an LCD-style readout showing the current key, BPM, and patch. The chord reference strip (`I C / ii Dm / iii Em ...`) tucks just under the top bar when chord mode is on. An adaptive timeline ruler underneath shows bar / beat / sub-beat ticks that thicken as you pinch in; tap to set a teal selector marker, drag to define a loop region with amber endpoint handles. PLAY honors the selector so you can practice a passage by tapping a starting point and pressing play repeatedly. Loop state is shared with the Studio screen so you can jump back and forth without losing your loop. Pitch and beat grid lines stay readable across all themes, including under clip and loop shading. Below the grid, a velocity strip renders one bar per note in track color. The bottom panel uses a fixed envelope so the GRID strip and tab bar never shift between tabs. Tab bar pinned to the bottom edge: TOOLS (DRAW / SELECT / ERASE editor modes plus COPY / UNDO / REDO actions), SCALE (ROOT knob, SCALE knob with OFF as the first detent, CHORD stepper that cycles OFF / TRIAD / 7TH / 9TH), EDIT (QUANT, VELOC latch, SUSTAIN reserved), INSTR (patch stepper plus a BROWSE button that slides the full categorized picker up over the grid). A latching segmented `1/4 1/8 1/16 1/32` GRID strip and a SNAP toggle sit above the tab content, always visible. Notes can be resized from either edge; a long-press grace zone extends a few pixels past the visible edge so resize gestures that overshoot still grip the handle, while quick taps in the same strip stay free to place notes in DRAW. In SELECT mode a long-press on empty grid starts a marquee for batch selection.
- **Quantize, velocity editing, marquee selection** -- the EDIT tab's QUANT button snaps each selected note's start to the nearest grid line at the active resolution, all in a single undo entry. Latching VELOC turns the velocity strip below the grid into an editable surface for currently-selected notes; drag any selected bar up or down and the same delta applies to every selected note's velocity with independent clamping at 0 and 1. With the SELECT tool active, long-press on empty grid and drag to paint a dashed amber marquee; release commits a fresh selection. Quick drags fall through to the parent scroll so you can still pan the grid in any tool. DRAW and ERASE never marquee, so a stray hold near a note edge can't sweep up a phantom selection.
...
- **Clip selection and actions** -- tap any clip (drum, MIDI, or audio) on the timeline to select it (amber border). Tap again to flip it over with a smooth X-axis rotation, revealing icon-only action buttons (duplicate, edit, rename, unlink, delete) on the back face. Tap a third time to unflip. Tapping the ruler, a track header, or a different clip deselects. Selecting an audio clip with multiple takes expands the take list below. Selecting a drum or MIDI clip updates the drawer's pattern editor or inline piano roll.
- **Linked clips and split** -- duplicate a clip as linked (same content, edits propagate) or unlinked (independent copy) via a two-half Duplicate pillrocker in the controls drawer. Every clip in a linked group shows a palette-colored strip across its top edge with a small chain glyph, so siblings are identifiable at a glance. Any propagating edit (trim, take switch, rename, split, overdub) flashes the strip on every sibling so propagation is visible, not silent. Split any clip non-destructively at a chosen point via a Split pillrocker (latch-to-arm + Confirm). Split propagates across linked groups so a whole group splits together into two linked groups. Unlink a clip from its group from the flip-panel action row. Rename the clip itself (not just its takes), with the new name propagating across the group.
- **Uniform clip length model** -- every clip carries an explicit authoritative length (ms for audio and MIDI, steps for drum) separate from its content. Split produces true sub-measure halves on all three clip types, drum patterns support sub-bar lengths natively (no more whole-bar lock), and notes or steps that fall outside a clip's window are preserved in the database but hidden and silent (re-expanding the clip restores them). Foundation for upcoming trim, extend, and glue actions.
...
- Linked clips (audio, MIDI, drum): opt-in "same content everywhere" duplicates with a unified `ClipLinkage` model (sourceClipId for audio/MIDI, shared patternId for drums). Propagating edits emit a sibling-pulse on every member of the group
- Non-destructive clip split at a chosen interior point, operating on the active take only; propagates across linked groups so a whole group splits into two linked groups. Source promotion on delete so takes and audio are preserved when the source of a linked group is removed
