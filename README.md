...
- **Full-screen piano roll** -- a redesigned hardware-style editor that reads as a Roland or Korg groovebox faceplate. A captioned top bar (BACK, RESTART, PLAY) sits above an LCD-style readout showing the current key, BPM, and patch. The diatonic chord reference strip (`I C / ii Dm / iii Em ...`) tucks just under the top bar whenever scale mode is on. An adaptive timeline ruler underneath shows bar / beat / sub-beat ticks that thicken as you pinch in; tap to set a teal selector + jump the playhead, drag to define a loop region with amber endpoint handles. Loop state is shared with the Studio screen so you can jump back and forth without losing your loop. Below the grid, a velocity strip renders one bar per note in track color so you can see your dynamics at a glance. Below that, four MODE tabs swap the control surface in groovebox style: TOOLS (split, erase, copy, undo, redo), SCALE (root and scale knobs with LCD readouts, hilite and chord toggles, chord-type knob), EDIT (quantize, velocity edit latch, sustain reserved), INSTR (slides the instrument picker into view in place of the grid). A persistent chip row keeps `1/2 1/4 1/8 1/16 1/32` resolution selection and a SNAP toggle one tap away from any tab. Restart preserves playback state and returns the playhead to the natural restart point: loop start if loop is on, else the selector, else 0. Notes can be resized from either edge, and long-press + drag on empty grid paints a dashed marquee for box-selection.
