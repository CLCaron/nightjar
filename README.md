...
- **MIDI instrument tracks** -- compose melodies, basslines, and chord progressions with 128 General MIDI instruments. Full-screen piano roll editor with tap-to-place notes, long-press-to-delete, snap-to-beat, and live playback. Inline MiniPianoRoll in the track drawer renders the whole track at once with the selected clip bright and other clips dimmed, so you can see and edit notes across every clip without re-selecting from the timeline. Taps land in whichever clip contains them; gap regions between clips are visibly darkened as non-writeable zones. Curated instrument picker with audition preview. Compact note visualization on the Studio timeline. Powered by FluidSynth synthesis via a dedicated C++ MidiSequencer.
